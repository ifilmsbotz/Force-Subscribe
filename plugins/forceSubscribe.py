    ...
    if chat_member.restricted_by:
      if chat_member.restricted_by.id == (client.get_me()).id:
          try:
            ...
            client.answer_callback_query(cb.id, text="I Like Your Smartness, But Don't Be Oversmart 😒", show_alert=True)
      else:
  ...
  if chat_db:
    ...
    if not client.get_chat_member(chat_id, user_id).status in ("administrator", "creator") and not user_id in Config.SUDO_USERS:
      ...
      try:
        ...
        try:
          buttons = [[
              InlineKeyboardButton('📢 Jᴏɪɴ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ!', url=f"https://t.me/{channel}")
          ],[
              InlineKeyboardButton('🔄 Tʀʏ Aɢᴀɪɴ!', callback_data='onUnMuteRequest')
          ]]
          reply_markup = InlineKeyboardMarkup(buttons)
          sent_message = message.reply_text(
              "**Hᴇʏ {} 🙋‍♂**\n\n**Pʟᴇᴀsᴇ Jᴏɪɴ Oᴜʀ Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ Tᴏ Usᴇ Tʜɪs Gʀᴏᴜᴘ!**\n\n**𝖠ғᴛᴇʀ Jᴏɪɴɪɴɢ Cʟɪᴄᴋ Oɴ Tʀʏ Aɢᴀɪɴ !**".format(message.from_user.mention),
